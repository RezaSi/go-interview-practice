...
func NewContentAggregator(fetcher ContentFetcher, processor ContentProcessor, workerCount int, requestsPerSecond int) *ContentAggregator {
	...
	return &ContentAggregator{fetcher: fetcher, processor: processor, workerCount: workerCount, limiter: rate.NewLimiter(rate.Limit(requestsPerSecond), requestsPerSecond), shutdown: make(chan struct{})}
}
...
func main() {
	...
	urls := []string{"", "https://example.org", "https://example.net"}
	startTime := time.Now()

...
type BankAccount struct {
	...
	Balance    float64
	MinBalance float64
	mu         sync.Mutex // no need to initilize
	// id         string
	// owner      string
	// balance    float64
	// minBalance float64
...
/*
Transfer modifies two accounts with only one lock. we must use 2 locks
*/
...
func (a *BankAccount) Transfer(amount float64, target *BankAccount) error {
	...
	/*
			Critical: Deadlock vulnerability in concurrent transfers.
			The current implementation can deadlock when two goroutines perform transfers between the same accounts in opposite directions:
		    Goroutine 1: accountA.Transfer(100, accountB) locks A, then B
		    Goroutine 2: accountB.Transfer(50, accountA) locks B, then A
		    Deadlock: Each goroutine holds one lock and waits for the other.
	*/
	if uintptr(unsafe.Pointer(a)) > uintptr(unsafe.Pointer(target)) {
		first, second = target, a
	}
	first.mu.Lock()
	defer first.mu.Unlock()
	if first != second { // Avoid double-locking same account
		second.mu.Lock()
	...
	if target.Balance+amount > MaxTransactionAmount {
		return &ExceedsLimitError{Value: target.Balance + amount}
	}
	a.Balance -= amount

...
func NaivePatternMatch(text, pattern string) []int {
	...
	for startIndex < len(text) {
	    index := strings.Index(text[startIndex:], pattern)
	    if index == -1 {
	        break
	    }
	    result = append(result, startIndex + index)
	    startIndex += index + 1
	}
	return result
}
...
func RabinKarpSearch(text, pattern string) []int {
	...
	mmax := max(len(text), len(pattern))
	pows := make([]int64, mmax, mmax)
	pows[0] = 1;
	for i := 1; i < mmax; i++ {
	    pows[i] = (pows[i - 1] * p) % mod
	}
	ht := make([]int64, len(text), len(text))
	for i, ch := range text {

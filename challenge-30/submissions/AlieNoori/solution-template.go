...
func SimulateWork(ctx context.Context, workDuration time.Duration, description string) error {
	...
	return ctxManager.WaitForCompletion(ctx, workDuration)
}
	...
	go func() {
		...
		for _, item := range items {
			...
			case <-ctx.Done():
				errChan <- ctx.Err()
				return
			case result <- fmt.Sprintf("processed_%s", item):
			}
		}
	}()

	out := []string{}
	for processedItem := range result {
		out = append(out, processedItem)
	}

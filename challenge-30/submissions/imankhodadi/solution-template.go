...
func ProcessItemsConcurrently(ctx context.Context, items []string, maxWorkers int) ([]string, error) {
		...
		go func() {
			...
			for work := range itemChan {
				...
				case <-ctx.Done():
					...
					return
				default:
					// Process item, here the result is a dummy value, later change it so that it can capture errors
	...
	for result := range resultChan {
		// in this assignment this code is unreachable. if processed items receive errors, this condition will work
		if result.err != nil {
...
// Chain multiple operations with context, function signature is part of the assignment, later change to func(context.Context)
...
// Rate limited context operations, function signature is part of the assignment, later change to func(context.Context)
